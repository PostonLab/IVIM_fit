...
if config["use_eddy_gpu"]:
    ...
    rule run_eddy_gpu:
        input:
            ...
            dwi_concat=rules.concat_dwi.output.dwi_concat,
...
rule cp_eddy_outputs:
    input:
        ...
        bval=bids(
            root=work, suffix="dwi.bval", datatype="dwi", **subj_wildcards
        ),
        mask=get_b0_mask(),
    output:
        multiext(
...
rule eddy_quad:
    input:
        ...
        bvecs=bids(
            root=work, suffix="dwi.bvec", datatype="dwi", **subj_wildcards
        ),
        eddy_dir=bids(
            root=work, suffix="eddy", datatype="dwi", **subj_wildcards
        ),
